...
import type { Task, ReviewArtifact, WSEvent } from '../../types'
import { api } from '../../api'
...
export function TaskDetailModal({
  ...
  const [selectedTool, setSelectedTool] = useState(task.assigned_tool ?? '')
  ...
  const selectedModelsQuery = useModelsQuery(selectedTool || undefined)
  ...
  const reviewSprintId =
    task.status === 'completed' && effectiveSprintId ? effectiveSprintId : ''
  ...
  const taskModels = selectedTool
    ? (selectedModelsQuery.data?.[selectedTool] ?? [])
    : []
  ...
  useEffect(() => {
    ...
    setSelectedTool(task.assigned_tool ?? '')
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[90vh] w-[560px] max-w-[95vw] flex-col overflow-y-auto rounded-[var(--radius)] border border-[var(--border)] bg-[var(--bg-primary)] shadow-[0_8px_32px_rgba(0,0,0,0.16)]"
              ...
              {(field) => (
                <label className="flex flex-col gap-1.5 text-xs font-medium text-[var(--text-secondary)]">
                  ...
                  <select
                    ...
                    onBlur={field.handleBlur}
                    onChange={(e) => field.handleChange(e.target.value)}
                    disabled={!selectedTool || selectedModelsQuery.isFetching}
                  >
          ...
          <div className="flex flex-col gap-2.5 rounded-md border border-[var(--border)] bg-[var(--bg-secondary)] p-3">
              ...
              <div className="flex gap-2">
                ...
                {hasPlan && (
                  <ActionButton
                    ...
                    disabled={planGenerating || generatePlanMutation.isPending}
                  >
            ...
            <div className="grid grid-cols-1 items-center gap-2 sm:grid-cols-[1fr_1fr_auto]">
              ...
              <select
                ...
                disabled={!generateTool || generateModelsQuery.isFetching}
              >
