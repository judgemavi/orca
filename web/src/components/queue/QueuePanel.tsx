...
import { useCancelJob, useDrainQueue, useQueueCounts, useQueueQuery } from '../../hooks/useQueue'
...
function JobRow({ job, onCancel, cancelling }: { job: Job; onCancel: (id: string) => void; cancelling: boolean }) {
  return (
    <tr className="border-b transition-colors hover:bg-muted/10 last:border-b-0">
      ...
      <td className="p-2 text-xs">
        {job.taskId ? (
          <Link to="/$taskId" params={{ taskId: job.taskId }} className="font-mono text-accent hover:underline">
      ...
      <td className="p-2 text-xs text-muted whitespace-nowrap">{relativeTime(job.createdAt)}</td>
      <td className="p-2 text-xs text-muted whitespace-nowrap">{relativeTime(job.startedAt)}</td>
      <td className="p-2">
        {job.status === 'queued' ? (
          <Button
...
export function QueuePanel() {
  ...
  const handleDrain = () => {
    drainMutation.mutate(undefined, {
      onSuccess: (result) => toast.success(`Drained ${result.cancelled} queued jobs`),
      onError: (err) => toast.error(err instanceof Error ? err.message : 'Drain failed'),
    })
  }
    ...
    <div className="flex min-h-0 flex-1 flex-col py-4">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-2 text-center">
          <p className="text-sm text-muted">No jobs{filter !== 'all' ? ` with status "${filter}"` : ''}.</p>
        </div>
      ) : (
          ...
          <table className="w-full min-w-[640px]">
            <thead>
              <tr className="border-b">
                {['Type', 'Task', 'Status', 'Priority', 'Created', 'Started', ''].map((h) => (
                  <th key={h} className="h-9 px-2 text-left align-middle text-xs font-medium whitespace-nowrap text-muted">
                    {h}
                  </th>
                ))}
              </tr>
