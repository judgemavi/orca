import { useEffect, useMemo, useRef, useState } from 'react'
import ReactMarkdown from 'react-markdown'
import remarkGfm from 'remark-gfm'
import { Button } from '../Button'
import { useOrchestratorChat } from '../../hooks/useOrchestratorChat'
import { detectAction } from '../../lib/orchestratorActions'
import { normalizeToolName } from '../../lib/orchestratorRichContent'
import {
  detectPhaseActions,
  parseToolResultPayload,
} from '../../lib/orchestratorPhaseActions'
import type { OrchestratorMessage } from '../../types'
import { ActionButtons } from './ActionButtons'
import { InlineTaskCard } from './InlineTaskCard'
import { ToolCallCard } from './ToolCallCard'

interface Props {
  className?: string
}

interface MessageEntry {
  message: OrchestratorMessage
  matchedToolResult?: OrchestratorMessage
}

interface AssistantCodeProps {
  children?: React.ReactNode
  className?: string
}

const TASK_ID_RE = /^[a-f0-9]{8,64}$/i
const TOOL_NAME_RE = /^(mcp__orca__)?[a-z][a-z0-9]*(?:_[a-z0-9]+)+$/i
const FILE_REF_RE = /^([A-Za-z0-9_./-]+\.[A-Za-z0-9]+)(?::\d+)?$/

function asString(value: unknown): string {
  return typeof value === 'string' ? value : ''
}

function asInlineToken(children: React.ReactNode): string {
  if (typeof children === 'string') return children.trim()
  if (Array.isArray(children)) {
    return children
      .map((child) => (typeof child === 'string' ? child : ''))
      .join('')
      .trim()
  }
  return ''
}

function latestActionableAssistantMessageID(
  entries: MessageEntry[],
  actedMessages: Set<string>,
  deletedTaskIds: Set<string>,
): string | null {
  for (let i = entries.length - 1; i >= 0; i -= 1) {
    const message = entries[i].message
    if (message.role !== 'assistant') continue
    if (actedMessages.has(message.id)) continue
    if (!detectAction(message.content)) continue
    const taskId = asString(message.metadata.task_id)
    if (taskId && deletedTaskIds.has(taskId)) continue
    return message.id
  }
  return null
}

function latestActionableToolUseMessageID(
  entries: MessageEntry[],
  actedMessages: Set<string>,
  deletedTaskIds: Set<string>,
): string | null {
  for (let i = entries.length - 1; i >= 0; i -= 1) {
    const entry = entries[i]
    const message = entry.message
    if (message.role !== 'tool_use') continue
    if (actedMessages.has(message.id)) continue
    if (!entry.matchedToolResult) continue
    if (entry.matchedToolResult.metadata.is_error) continue

    const toolName = normalizeToolName(message.content)
    const payload = parseToolResultPayload(toolName, entry.matchedToolResult.content)
    const actionSet = detectPhaseActions(toolName, payload)
    if (!actionSet || actionSet.actions.length === 0) continue

    const taskId = asString(entry.matchedToolResult.metadata.task_id)
    if (taskId && deletedTaskIds.has(taskId)) continue
    return message.id
  }
  return null
}

function buildMessageEntries(messages: OrchestratorMessage[]): MessageEntry[] {
  const consumedToolResultIDs = new Set<string>()
  const entries: MessageEntry[] = []

  for (let i = 0; i < messages.length; i += 1) {
    const message = messages[i]
    if (
      message.role === 'tool_result' &&
      consumedToolResultIDs.has(message.id)
    ) {
      continue
    }

    if (message.role !== 'tool_use') {
      entries.push({ message })
      continue
    }

    const toolUseID = asString(message.metadata.tool_use_id)
    let matchedToolResult: OrchestratorMessage | undefined

    for (let j = i + 1; j < messages.length; j += 1) {
      const candidate = messages[j]
      if (candidate.role !== 'tool_result') continue
      if (consumedToolResultIDs.has(candidate.id)) continue

      const resultToolUseID = asString(candidate.metadata.tool_use_id)
      const resultName = asString(candidate.metadata.name)
      const matchesByID =
        toolUseID !== '' &&
        resultToolUseID !== '' &&
        resultToolUseID === toolUseID
      const matchesByName =
        resultName !== '' && resultName === message.content

      if (!matchesByID && !matchesByName) continue

      matchedToolResult = candidate
      consumedToolResultIDs.add(candidate.id)
      break
    }

    entries.push({ message, matchedToolResult })
  }

  return entries
}

export function ChatPane({ className }: Props) {
  const [draft, setDraft] = useState('')
  const {
    messages,
    actedMessages,
    deletedTaskIds,
    isLoadingHistory,
    isStreaming,
    error,
    sendMessage,
    handleAction,
    newSession,
  } = useOrchestratorChat()
  const endRef = useRef<HTMLDivElement>(null)
  const entries = useMemo(() => buildMessageEntries(messages), [messages])
  const latestActionableID = useMemo(
    () => latestActionableAssistantMessageID(entries, actedMessages, deletedTaskIds),
    [entries, actedMessages, deletedTaskIds],
  )
  const latestActionableToolUseID = useMemo(
    () => latestActionableToolUseMessageID(entries, actedMessages, deletedTaskIds),
    [entries, actedMessages, deletedTaskIds],
  )
  useEffect(() => {
    if (entries.length === 0) return
    endRef.current?.scrollIntoView({ block: 'end' })
  }, [entries])

  const handleSend = async () => {
    const text = draft.trim()
    if (!text || isStreaming) return
    setDraft('')
    await sendMessage(text)
  }

  return (
    <div className={`flex h-full min-h-0 flex-col ${className ?? ''}`}>
      <div className="flex items-center justify-between gap-2 border-b border-border-subtle px-3 py-2">
        <span className="text-xs font-medium text-muted">
          Orchestrator Chat
        </span>
        <Button
          className="text-xs"
          onClick={() => void newSession()}
          disabled={isStreaming}
        >
          New Session
        </Button>
      </div>

      <div className="flex-1 overflow-y-auto px-3 py-3">
        {isLoadingHistory ? (
          <div className="text-xs text-muted">Loading history...</div>
        ) : null}
        <div className="flex flex-col gap-3">
          {entries.map(({ message, matchedToolResult }) => {
            if (message.role === 'user') {
              return (
                <div key={message.id} className="flex justify-end">
                  <div className="max-w-[85%] rounded-lg bg-surface-alt px-3 py-2 text-sm whitespace-pre-wrap break-words">
                    {message.content}
                  </div>
                </div>
              )
            }

            if (message.role === 'assistant') {
              const action = detectAction(message.content)
              const showActionButtons =
                action !== null &&
                latestActionableToolUseID === null &&
                message.id === latestActionableID &&
                !actedMessages.has(message.id)
              return (
                <div key={message.id} className="flex justify-start">
                  <div className="max-w-[92%] rounded-lg bg-surface px-3 py-2">
                    <div className="prose prose-sm max-w-none dark:prose-invert">
                      <ReactMarkdown
                        remarkPlugins={[remarkGfm]}
                        components={{
                          code: ({ children, className }: AssistantCodeProps) => {
                            const token = asInlineToken(children)
                            if (TASK_ID_RE.test(token)) {
                              return <InlineTaskCard taskId={token} />
                            }
                            if (TOOL_NAME_RE.test(token)) {
                              return (
                                <span className="rounded bg-surface-alt px-1.5 py-0.5 font-mono text-[11px]">
                                  {normalizeToolName(token)}
                                </span>
                              )
                            }
                            if (FILE_REF_RE.test(token)) {
                              return (
                                <code className="rounded bg-surface-alt px-1.5 py-0.5 font-mono text-[11px]">
                                  {token}
                                </code>
                              )
                            }
                            return (
                              <code className={className}>
                                {children}
                              </code>
                            )
                          },
                        }}
                      >
                        {message.content}
                      </ReactMarkdown>
                    </div>
                    {showActionButtons ? (
                      <ActionButtons
                        action={action}
                        messageId={message.id}
                        disabled={isStreaming}
                        onAction={(response) =>
                          handleAction(message.id, response)
                        }
                      />
                    ) : null}
                  </div>
                </div>
              )
            }

            if (message.role === 'tool_use') {
              return (
                <div key={message.id} className="flex justify-start">
                  <ToolCallCard
                    name={message.content}
                    toolUseId={asString(message.metadata.tool_use_id)}
                    actionMessageId={message.id}
                    showPhaseActions={
                      message.id === latestActionableToolUseID &&
                      !actedMessages.has(message.id)
                    }
                    actionsDisabled={isStreaming}
                    onAction={(response) => handleAction(message.id, response)}
                    args={asString(message.metadata.args) || '{}'}
                    result={
                      matchedToolResult
                        ? {
                            content: matchedToolResult.content,
                            isError: Boolean(
                              matchedToolResult.metadata.is_error,
                            ),
                          }
                        : undefined
                    }
                  />
                </div>
              )
            }

            return (
              <div key={message.id} className="flex justify-start">
                <div className="w-full max-w-[92%] rounded-lg border border-border-subtle bg-surface-alt/60 px-3 py-2">
                  <div className="mb-1 text-[11px] uppercase tracking-wide text-muted">
                    Tool Result
                  </div>
                  <pre className="max-h-52 overflow-auto font-mono text-[11px] whitespace-pre-wrap break-words">
                    {message.content || '(empty result)'}
                  </pre>
                </div>
              </div>
            )
          })}

          {isStreaming ? (
            <div className="text-xs text-muted">
              <span className="inline-block animate-pulse">
                Assistant is responding...
              </span>
            </div>
          ) : null}
        </div>
        <div ref={endRef} />
      </div>

      {error ? (
        <div className="border-t border-border-subtle px-3 py-2 text-xs text-danger">
          {error}
        </div>
      ) : null}

      <div className="border-t border-border-subtle px-3 py-3">
        <div className="flex items-end gap-2">
          <textarea
            value={draft}
            onChange={(event) => setDraft(event.target.value)}
            onKeyDown={(event) => {
              if (event.key === 'Enter' && !event.shiftKey) {
                event.preventDefault()
                void handleSend()
              }
            }}
            className="min-h-20 max-h-56 flex-1 resize-y rounded-md border border-border-subtle bg-surface px-2.5 py-2 text-sm outline-none focus:border-accent"
            placeholder="Ask the orchestrator..."
            disabled={isStreaming}
          />
          <Button
            variant="primary"
            onClick={() => void handleSend()}
            disabled={!draft.trim() || isStreaming}
          >
            Send
          </Button>
        </div>
      </div>
    </div>
  )
}
