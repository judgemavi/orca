...
import { api } from '../api';
import type { OrchestratorMessage, OrchestratorSessionSummary } from '../types';
...
function extractTaskIdFromJSON(text: string): string | null {
  try {
    ...
    if (typeof obj?.taskId === 'string' && obj.taskId.trim())
      return obj.taskId.trim();
    if (typeof obj?.task?.id === 'string' && obj.task.id.trim())
      return obj.task.id.trim();
  } catch {
    /* not JSON */
  }
  return null;
...
export function useOrchestratorChat() {
  ...
  const [sessions, setSessions] = useState<OrchestratorSessionSummary[]>([]);
  ...
  const loadSessions = useCallback(async () => {
    try {
      const list = await api.listOrchestratorSessions();
      setSessions(list);
    } catch {}
  }, []);
        ...
        const handleEvent = (evt: ParsedSSEEvent) => {
          switch (evt.event) {
            ...
            case 'tool_result': {
              ...
              setMessages((prev) => [
                ...prev,
                nextLocalMessage('tool_result', contentStr, {
  ...
  const switchSession = useCallback(
    async (sessionId: string) => {
      ...
      setError(null);
      try {
        await api.resumeOrchestratorSession(sessionId);
        await loadHistory();
        await loadSessions();
      } catch (err) {
        setError(getErrorMessage(err));
    ...
    [isStreaming, loadHistory, loadSessions],
  );
