...
func (s *Store) scanTask(query string, args ...interface{}) (*Task, error) {
	...
	var t Task
	var desc, prompt, model, phaseConfig, plan, sessionID, parentID, tool *string
	err := row.Scan(&t.ID, &t.Title, &desc, &prompt, &model, &phaseConfig, &plan, &sessionID, &parentID, &t.Status, &tool, &t.CreatedAt, &t.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func (s *Store) queryTasks(query string, args ...interface{}) ([]*Task, error) {
	...
	for rows.Next() {
		var t Task
		var desc, prompt, model, phaseConfig, plan, sessionID, parentID, tool *string
		if err := rows.Scan(&t.ID, &t.Title, &desc, &prompt, &model, &phaseConfig, &plan, &sessionID, &parentID, &t.Status, &tool, &t.CreatedAt, &t.UpdatedAt); err != nil {
