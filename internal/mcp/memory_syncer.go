...
func (s *Server) newMemorySyncer(store *memory.Store) *memory.Syncer {
	...
	if s.config != nil {
		if toolName, tool, err := s.config.ResolveToolForPhase(interaction.PhaseExplore, ""); err == nil {
			model := s.config.ResolveModelForPhase(interaction.PhaseExplore, "", toolName)
