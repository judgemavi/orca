...
6. `retro` — extract reusable knowledge from approved/merged tasks
7. `merge` — merge approved tasks into integration branch

Task flow: `pending → planned → running → review → approved → merged`.
Post-completion phase: `approved/merged → retro` (knowledge extraction via CLI/MCP/API).
...
- RAG knowledge system: retro extracts reusable patterns, pitfalls, preferences, and conventions
- Knowledge-informed planning with FTS5/BM25 retrieval
- Provenance-aware knowledge lifecycle (provenance hashes, supersession, confidence decay)
- MCP server for agentic orchestration (40 tools)
...
`orca mcp` exposes 40 MCP tools for task orchestration:
...
- **Retro:** `tasks_retro`
- **Knowledge:** `knowledge_list`, `knowledge_get`, `knowledge_search`, `knowledge_update`, `knowledge_delete`
...
internal/
  ...
  knowledge/        RAG knowledge store + FTS5 search
