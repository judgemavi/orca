...
export class DriverRegistry {
  ...
  async loadUserDrivers(dir: string): Promise<void> {
    ...
    for (const file of candidates) {
      ...
      if (!isDriver(maybe)) {
        throw new Error(`user driver ${file} does not implement Driver interface`)
      }
...
export async function loadDriverRegistry(repoDir: string): Promise<DriverRegistry> {
...
export function toolDefinition(registry: DriverRegistry, name: string): Driver | null {
