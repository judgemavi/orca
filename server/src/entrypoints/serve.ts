...
import { failInFlightForShutdown, runStartupRecovery } from '../domain/recovery'
...
export async function runServeEntrypoint(repoDir: string, port: number) {
  ...
  await runStartupRecovery(ctx.taskStore, ctx.interactionStore, (event, data) => {
  ...
  const shutdown = async () => {
    ...
    await failInFlightForShutdown(ctx.taskStore, ctx.interactionStore, (event, data) => {
      console.error(`[${event}]`, data ?? {})
    })
    ctx.database.close()
    process.exit(0)
