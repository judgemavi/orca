...
import { JOB_PRIORITIES, JobStatus, JobType } from '../types'
import type { Job, JobStatus as JobStatusType, JobType as JobTypeType } from '../types'
...
export class JobQueue {
    ...
    return this.db.transaction(async (tx) => {
      ...
      for (const jobId of ids) {
        await tx
          ...
          .where(and(eq(jobsTable.id, jobId), eq(jobsTable.status, JobStatus.queued)))
        const rows = await tx
          .select()
          .from(jobsTable)
  ...
  async cancelForTask(taskId: string): Promise<number> {
    const result = await this.db
      .update(jobsTable)
      .set({ status: JobStatus.cancelled })
      .where(and(eq(jobsTable.taskId, taskId), eq(jobsTable.status, JobStatus.queued)))
