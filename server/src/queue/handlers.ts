...
export function registerJobHandlers(processor: JobProcessor, deps: HandlerDeps): void {
  // evaluate
  ...
  processor.register('review', async (job: Job) => {
    ...
    try {
      ...
      return { approved: result.approved, feedback: result.feedback, reviewId: result.reviewId }
    } catch (err) {
      const error = err instanceof Error ? err.message : String(err)
