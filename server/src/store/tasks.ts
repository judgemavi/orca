...
export class TaskStore {
  ...
  async resolveID(prefix: string): Promise<string> {
    ...
    if (rows.length === 0) throw new Error(`no task matching prefix ${JSON.stringify(normalized)}`)
    if (rows.length > 1) {
      throw new Error(`ambiguous prefix ${JSON.stringify(normalized)} matches ${rows.length} tasks`)
    }
    return rows[0].id
  }
  ...
  async delete(id: string): Promise<void> {
    ...
    if (!DELETABLE_STATUS.has(task.status)) {
      throw new Error(`cannot delete task in ${JSON.stringify(task.status)} status`)
    }
    ...
    if (dependentCount > 0) {
      throw new Error(`cannot delete task: ${dependentCount} other task(s) depend on it`)
    }

    await this.db.transaction(async (tx) => {
      await tx.delete(taskReviewsTable).where(eq(taskReviewsTable.taskId, id))
      await tx.delete(taskInteractionsTable).where(eq(taskInteractionsTable.taskId, id))
      await tx.delete(taskDepsTable).where(eq(taskDepsTable.taskId, id))
      await tx.delete(taskFileAssociationsTable).where(eq(taskFileAssociationsTable.taskId, id))
  ...
  async removeDependency(taskID: string, dependsOnID: string): Promise<void> {
    await this.db
      ...
      .where(and(eq(taskDepsTable.taskId, taskID), eq(taskDepsTable.dependsOn, dependsOnID)))
  }
  ...
  async updateDependencies(taskID: string, deps: string[]): Promise<void> {
    ...
    const cycle = detectCycle([...graph.keys()], (nodeID) => graph.get(nodeID) ?? [])
    if (cycle) {
    ...
    await this.db.transaction(async (tx) => {
      ...
      for (const dep of normalized) {
        await tx.insert(taskDepsTable).values({ taskId: taskID, dependsOn: dep })
      }
    })
  }
  ...
  async getReady(): Promise<Task[]> {
    ...
    const rows = await this.db
      .select()
      .from(tasksTable)
      .where(and(eq(tasksTable.status, TaskStatusValue.planned), notExists(blockingDeps)))
      .orderBy(asc(tasksTable.createdAt))
    const deps = await this.loadDepsForTaskIDs(rows.map((row) => row.id))
    return rows.map((row) => this.mapTask(row, deps.get(row.id) ?? []))
  }
  ...
  async addReview(taskID: string, feedback: string, interactionID = ''): Promise<string> {
    const id = crypto.randomUUID()
    await this.db
      .insert(taskReviewsTable)
      .values({
        id,
        taskId: taskID,
  ...
  async getPendingReview(taskID: string): Promise<{ id: string; feedback: string } | null> {
    const rows = await this.db
      .select({
  ...
  async addressReview(reviewID: string): Promise<void> {
    await this.db
      ...
      .set({ status: ReviewStatusValue.addressed, addressedAt: sql`(CURRENT_TIMESTAMP)` })
  ...
  async associateFiles(taskID: string, paths: string[]): Promise<void> {
    ...
    for (const path of normalized) {
      await this.db
        ...
        .onConflictDoNothing({
          target: [taskFileAssociationsTable.taskId, taskFileAssociationsTable.filePath],
        })
    }
  }
  ...
  async findTasksByFilePaths(paths: string[], excludeStatuses: string[] = []): Promise<string[]> {
    ...
    const rows = await this.db
      ...
      .from(taskFileAssociationsTable)
      .innerJoin(tasksTable, eq(tasksTable.id, taskFileAssociationsTable.taskId))
      .where(whereClause)
  ...
  private async loadDepsForTaskIDs(taskIDs: string[]): Promise<Map<string, string[]>> {
    ...
    for (const [id, deps] of out.entries()) {
      out.set(id, [...deps].sort((a, b) => a.localeCompare(b)))
    }
