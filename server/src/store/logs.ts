...
export class RotatingLogWriter {
  ...
  async writeLine(jsonLine: string): Promise<void> {
    ...
    const prior = await Bun.file(this.path).text().catch(() => '')
...
export class LogStore {
  async query(logPath: string, filter: LogQueryFilter = {}): Promise<LogEntry[]> {
    ...
    for (const line of lines) {
      ...
      if (pattern) {
        ...
        if (!entry.msg.toLowerCase().includes(pattern) && !body.includes(pattern)) continue
      }
