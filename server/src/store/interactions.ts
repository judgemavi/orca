...
import { and, desc, eq, gte, isNotNull, isNull, like, ne, or, sql, type SQL } from 'drizzle-orm'
import type { OrcaDrizzleDB } from '../db/connection'
import { taskInteractions } from '../db/schema'
import {
  InteractionStatus as InteractionStatusValue,
  Phase,
} from '../types'
...
export class InteractionStore {
  ...
  async finish(id: string, fields: {
    ...
    if (fields.qualityJson !== undefined) updates.qualityJson = fields.qualityJson
    if (fields.inputTokens !== undefined) updates.inputTokens = fields.inputTokens
    if (fields.outputTokens !== undefined) updates.outputTokens = fields.outputTokens
    if (fields.estimatedCost !== undefined) updates.estimatedCost = fields.estimatedCost
  ...
  async listByPhase(taskID: string, phase: string): Promise<StoredInteraction[]> {
    const rows = await this.db
      .select()
      .from(taskInteractions)
      .where(and(eq(taskInteractions.taskId, taskID), eq(taskInteractions.phase, phase)))
      .orderBy(desc(taskInteractions.startedAt))
    return rows.map((row) => this.mapInteraction(row))
  }
  ...
  async projectTotal(): Promise<number> {
    const rows = await this.db
      .select({
        total: sql<number | null>`sum(${taskInteractions.estimatedCost})`.as('total'),
      })
      .from(taskInteractions)
  ...
  async runSummary(runID: string): Promise<ToolSummary[]> {
    const rows = await this.db
      .select({
        ...
        inputTokens: sql<number | null>`sum(${taskInteractions.inputTokens})`.as('inputTokens'),
        outputTokens: sql<number | null>`sum(${taskInteractions.outputTokens})`.as('outputTokens'),
        cost: sql<number | null>`sum(${taskInteractions.estimatedCost})`.as('cost'),
      })
      .from(taskInteractions)
      .where(eq(taskInteractions.runId, runID))
  ...
  async listRuns(limit = 20): Promise<RunCostSummary[]> {
    ...
    const rows = await this.db
      .select({
        ...
        outputTokens: sql<number | null>`sum(${taskInteractions.outputTokens})`.as('outputTokens'),
        cost: sql<number | null>`sum(${taskInteractions.estimatedCost})`.as('cost'),
        startedAt: sql<string>`min(${taskInteractions.startedAt})`.as('startedAt'),
  ...
  async listOperations(opts: { all?: boolean; sinceISO?: string; limit?: number } = {}): Promise<StoredInteraction[]> {
  ...
  private async nextAttempt(taskID: string | null, phase: string): Promise<number> {
    const rows = await this.db
      .select({
        attempt: sql<number>`coalesce(max(${taskInteractions.attempt}), 0) + 1`.as('attempt'),
      })
      .from(taskInteractions)
      .where(
        and(
          eq(taskInteractions.phase, phase),
          taskID === null ? isNull(taskInteractions.taskId) : eq(taskInteractions.taskId, taskID),
        ),
      )
