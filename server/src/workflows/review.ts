...
export async function runAIReviewWorkflow(
  ...
  const task = await getTaskInReview(
    taskID,
    deps.taskStore,
    (current) => `task must be in review status, got ${JSON.stringify(current.status)}`,
  )

  const runInteractions = await deps.interactions.listByPhase(taskID, Phase.run)
  ...
  let reviewID = ''
  if (!review.approved) {
    reviewID = await deps.taskStore.addReview(taskID, review.feedback, review.interactionId)
  }
