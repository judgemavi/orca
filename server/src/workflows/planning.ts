...
export interface RequestPlanChangesWorkflowDeps extends Omit<GeneratePlanWorkflowDeps, 'feedback'> {
  interactionId?: string
}

export interface RequestPlanChangesWorkflowResult extends GeneratePlanWorkflowResult {
  reviewId: string
}
...
export async function acceptBreakdown(
  ...
  if (normalizedParentID) {
    await deps.taskStore.updateStatus(normalizedParentID, TaskStatus.broken_down)
  }
...
export async function requestPlanChanges(
  ...
  const reviewID = await deps.taskStore.addReview(taskID, normalizedFeedback, deps.interactionId ?? '')
...
export async function loadProposedTasksFromInteraction(
  ...
  try {
    const parsed = JSON.parse(interaction.qualityJson) as { proposed?: ProposedTask[] }
...
async function retrievePlanningMemory(input: {
  ...
  const memory = await retrieveBudgetedMemory(memoryStore, input.taskStore, {
    ...
    filePaths: input.taskId ? await input.taskStore.getFilePaths(input.taskId) : [],
    syncer: {
...
function canRunLLMBreakdown(
  ...
  return Boolean(deps.repoDir && deps.interactions && deps.configStore && deps.registry)
}
