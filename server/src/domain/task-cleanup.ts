...
export async function cleanupTaskArtifacts(input: {
  ...
  const worktreePath = await findTaskWorktree(input.repoDir, taskID).catch((error) => {
    result.warnings.push(`failed to locate worktree: ${toErrorMessage(error)}`)
    return ''
  })
  ...
  const branches = await listTaskBranches(input.repoDir, taskID).catch((error) => {
  ...
  for (const branch of branches) {
    ...
    if (!isMissingBranchError(message)) {
      result.warnings.push(`branch delete failed for ${branch}: ${message || 'unknown error'}`)
    }
  }
...
export async function listTaskBranches(repoDir: string, taskID: string): Promise<string[]> {
  const output = await runGit(
    repoDir,
    ['for-each-ref', '--format=%(refname:short)', `refs/heads/orca/task-${taskID}*`],
    true,
  )
...
function isMissingBranchError(message: string): boolean {
  const normalized = message.toLowerCase()
  return normalized.includes('not found') || normalized.includes('unknown revision')
}
