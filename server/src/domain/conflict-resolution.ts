...
export interface ConflictResolutionRuntime {
  mergeTaskWithGitUnlocked: (taskID: string, deps: IntegratorDeps) => Promise<MergeResult>
  resolveTaskBranch: (repoDir: string, taskID: string, worktreePath: string) => Promise<string>
  runValidation: (repoDir: string, commands: string[]) => Promise<MergeValidationResult>
...
export async function mergeWithConflictResolutionUnlocked(
  ...
  if (basicResult.status === TaskStatus.merged || basicResult.conflicts.length === 0) {
    return basicResult
  }

  const worktreePath = await findTaskWorktree(deps.repoDir, taskID)
  const branch = await runtime.resolveTaskBranch(deps.repoDir, taskID, worktreePath)
  ...
  const model = resolveModelForPhase(deps.config, deps.registry, Phase.merge, toolName, '')
  ...
  try {
    const rebaseStart = await gitRunWithRefLockRetry(worktreePath, ['rebase', base])
    if (rebaseStart.code === 0) {
      const checkoutBase = await gitRunWithRefLockRetry(
        deps.repoDir,
        ['checkout', base],
      )
      ...
      const finalMerge = await gitRunWithRefLockRetry(
        deps.repoDir,
        ['merge', branch, '--no-ff', '-m', `orca: merge task-${taskID} (after rebase)`],
      )
      ...
      const validation = await runtime.runValidation(deps.repoDir, deps.validationCommands)
      ...
      await runtime.cleanupTaskWorktree(deps.repoDir, worktreePath, branch, taskID)
      await deps.interactionStore.finish(interaction.id, { status: InteractionStatus.completed, model })
      return {
        taskId: taskID,
    ...
    for (let attempt = 0; attempt < MAX_CONFLICT_ATTEMPTS; attempt++) {
      ...
      const cont = await gitRunWithRefLockRetry(
        worktreePath,
    ...
    if (stillConflicting.length > 0) {
      ...
      return failed(`conflict resolution failed: ${stillConflicting.length} files unresolved`, {
        conflicts: stillConflicting,
      })
    }

    const checkoutBase = await gitRunWithRefLockRetry(deps.repoDir, ['checkout', base])
    if (checkoutBase.code !== 0) {
      if (isRefLockErrorResult(checkoutBase)) {
        await deps.interactionStore.finish(interaction.id, {
          status: InteractionStatus.failed,
      ...
      return failed('checkout integration branch failed after conflict resolution')
    }

    const finalMerge = await gitRunWithRefLockRetry(
      deps.repoDir,
      ['merge', branch, '--no-ff', '-m', `orca: merge task-${taskID} (conflict-resolved)`],
    )
    if (finalMerge.code !== 0) {
      await runtime.gitRun(deps.repoDir, ['merge', '--abort'], true)
      if (isRefLockErrorResult(finalMerge)) {
    ...
    await runtime.gitRun(worktreePath, ['rebase', '--abort'], true).catch(() => {})
...
export async function readConflictFiles(repoDir: string): Promise<string[]> {
  const result = await gitRun(repoDir, ['diff', '--name-only', '--diff-filter=U'])
