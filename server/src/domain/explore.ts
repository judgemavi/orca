...
export async function runExplore(input: RunExploreInput): Promise<RunExploreResult> {
  ...
  const { result, interactionId, tool, model } = await runPhase(
    ...
    async (output) => {
      ...
      const seeded = await seedExploreMemory(input.repoDir, input.memory, context, extracted)
...
export async function readExploreContext(repoDir: string): Promise<string> {
  return Bun.file(exploreContextPath(repoDir)).text().catch(() => '')
}

export async function writeExploreContext(repoDir: string, content: string): Promise<string> {
...
async function buildExplorePrompt(
  ...
  const filesSection = trackedFiles.length === 0
    ? '(none)'
    : trackedFiles.slice(0, 4_000).map((file) => `- ${file}`).join('\n')
...
function buildExistingMemorySection(entries: Awaited<ReturnType<MemoryStore['list']>>): string {
...
async function seedExploreMemory(
  ...
  for (const row of extracted) {
    ...
    const provenanceHash = hashExploreSeed(contextContent, content, category, filePaths)
...
function filterTrackedFilePaths(paths: string[], tracked: Set<string>): string[] {
