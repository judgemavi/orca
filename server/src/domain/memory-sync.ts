...
import type { Config, MemoryEntry, MemoryRefreshResult, MemorySyncResult } from '../types'
import type { DriverRegistry } from '../driver/registry'
import type { InteractionStore } from '../store/interactions'
import { runTool } from '../worker/worker'
import { gitOutput } from '../shared/git'
import { classifyDiffFromGit, rankFor, type DiffChangeType } from './diff-classify'
...
export async function syncMemoryWithGit(
  ...
  const commitCountRaw = await gitOutput(repoDir, ['rev-list', '--count', `${lastSynced}..${currentCommit}`]).catch(() => '0')
  result.commitCount = Number.parseInt(commitCountRaw.trim(), 10) || 0

  const nameStatus = await gitOutput(repoDir, ['diff', '--name-status', `${lastSynced}..${currentCommit}`]).catch(() => '')
  const numStat = await gitOutput(repoDir, ['diff', '--numstat', `${lastSynced}..${currentCommit}`]).catch(() => '')
  ...
  for (const entry of entries) {
    const classification = classifyEntry(entry.filePaths ?? [], classifications)
...
export async function refreshMemoryEntries(
  ...
  if (entryID.trim()) {
    ...
    const updated = await refreshOne(repoDir, memory, entry.id, commit, options)
    return {
      entryId: entry.id,
      updated: updated ? 1 : 0,
      skipped: updated ? 0 : 1,
...
export async function getMemorySyncStatus(
  ...
  const head = (await gitOutput(repoDir, ['rev-parse', 'HEAD']).catch(() => '')).trim()
  const same = Boolean(last) && last === head

  const commitsBehindRaw = !last || same
    ? '0'
    : await gitOutput(repoDir, ['rev-list', '--count', `${last}..${head}`]).catch(() => '0')
...
async function tryPatchExploreContext(
  ...
  const diff = await gitOutput(repoDir, ['diff', `${lastCommit}..${headCommit}`]).catch(() => '')
...
async function refreshOne(
  ...
  const changed = await hasEntryChanges(repoDir, existingPaths, entry.coveredAtCommit ?? '', commit)
  ...
  if (llmResult.valid || !llmResult.updatedContent) {
    await memory.update(entry.id, {
      stale: false,
      coveredAtCommit: commit,
      confidence: deletedPaths.length > 0
  ...
  const nextFilePaths = llmResult.updatedFilePaths.length > 0
    ? llmResult.updatedFilePaths
    : existingPaths
  const nextConfidence = entry.confidence > 0
    ? Math.min(1, entry.confidence)
    : 0.95
  const replacement = await memory.create({
    ...
    provenanceHash: refreshProvenanceHash(entry, llmResult.updatedContent, commit),
  })
...
async function refreshEntryWithLLM(
  ...
  const currentFileContent = await collectCurrentFileContent(repoDir, existingPaths, maxDiffBytes)
...
async function collectCurrentFileContent(
  ...
  for (const path of normalizePaths(filePaths)) {
    const content = await gitOutput(repoDir, ['show', `HEAD:${path}`])
      .catch(async () => Bun.file(`${repoDir}/${path}`).text().catch(() => ''))
