...
export function analyzeScope(
  ...
  const topDirs = new Set(
    [...files].map((path) => (path.includes('/') ? path.slice(0, path.indexOf('/')) : '(root)')),
  )
  const testFiles = [...files].filter((path) => isTestFile(path)).length
  const fileCount = files.size
  const linesChanged = classified.totalChurn > 0 ? classified.totalChurn : added + removed
  const threshold = fileThreshold(title)
  const flags: string[] = []
  const refactorTask = /\b(refactor|rename)\b/i.test(title)

  if (fileCount > threshold) {
    flags.push(`${fileCount} files changed (expected <${threshold} for task size)`)
  }
...
export function computeTestDelta(
  ...
  const beforeByCommand = new Map(before.results.map((result) => [result.command, result]))
  let beforeTests = 0
  let afterTests = 0
  let hasCounts = true
...
export function evaluateQualityGates(input: {
  ...
  if (input.testDelta) {
    ...
    if (delta.newFailures.length > 0) {
      result.blockingIssues.push(`new validation failures: ${delta.newFailures.join(', ')}`)
    }
  }
