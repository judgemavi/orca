...
export function triggerPostMergeHooks(taskID: string, deps: PostMergeDeps): void {
    ...
    if (hooks.memorySync !== false) {
      try {
        ...
        const contextDeps = deps.registry
          ...
          : undefined
        const result = await syncMemoryWithGit(deps.repoDir, deps.memoryStore, contextDeps)
