...
export function resolveWorktreeRoot(repoDir: string, configuredPath: string): string {
...
export async function ensureIntegrationBranch(repoDir: string, branch: string): Promise<void> {
  ...
  if (create.exitCode !== 0 && !/already exists/i.test(create.stderr)) {
    throw new Error(`failed to create integration branch ${JSON.stringify(branch)}: ${create.stderr}`)
  }
}

export async function findTaskWorktree(repoDir: string, taskID: string): Promise<string> {
...
export async function ensureTaskWorktree(input: {
  ...
  const added = await gitRun(
    input.repoDir,
    ['worktree', 'add', worktreePath, '-b', branchName, input.integrationBranch],
    true,
  )
...
async function gitRun(
  ...
  if (!allowFailure && exitCode !== 0) {
    throw new Error(stderr || `git ${args.join(' ')} failed with exit code ${exitCode}`)
  }
