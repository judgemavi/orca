...
export async function runRetro(taskID: string, deps: RetroDeps): Promise<RetroResult> {
      ...
      async (output, context) => {
        ...
        for (const entry of normalized) {
          ...
          if (entry.supersedes?.trim()) {
            await deps.memoryStore.supersede(entry.supersedes, created.id).catch(() => {})
          }
        }

        const summary: RetroSummary = {
          highlights: normalized.length > 0
            ? normalized.map((e) => e.content.slice(0, 120))
            : [`Task ${task.title} completed — no durable memory extracted.`],
          improvement: normalized.length > 0
...
export function buildRetroSummary(task: Pick<Task, 'title' | 'status'>): RetroSummary {
  return {
    highlights: [
...
async function collectRetroContext(deps: RetroDeps, task: Task): Promise<RetroContext> {
  ...
  const planReviewFeedback = reviews
    .filter((r) => (r as any).phase === Phase.plan || r.feedback?.toLowerCase().includes('plan'))
    .map((r) => r.feedback?.trim())
    .filter(Boolean)
    .join('\n---\n')
  ...
  for (const pi of planInteractions) {
    ...
    try {
      const parsed = JSON.parse(pi.qualityJson) as {
        memory?: { entries?: Array<{ id?: string; provenanceHash?: string; content?: string }> }
        usedMemoryIds?: string[]
      }
      ...
      for (const entry of parsed.memory?.entries ?? []) {
        ...
        if (entry.provenanceHash) usedProvenanceHashes.push(entry.provenanceHash)
        if (entry.content) usedMemoryContent.push(entry.content)
      }
    } catch { /* ignore */ }
  }

  return { planDiffs, runDiffs, reviewFeedback, planReviewFeedback, usedMemoryIDs, usedProvenanceHashes, usedMemoryContent }
}

function retroProvenanceHash(planDiffs: string, runDiffs: string, reviewFeedback: string, planReviewFeedback: string): string {
  return createHash('sha256')
    .update([planDiffs, runDiffs, reviewFeedback, planReviewFeedback].join('\n---\n'))
    .digest('hex')
}

async function buildRetroPrompt(deps: RetroDeps, task: Task, ctx: RetroContext): Promise<string> {
...
async function loadRelatedMemory(memoryStore: MemoryStore, task: Task): Promise<string> {
  const entries = await memoryStore.search(task.title, 10).catch(() => [] as MemoryEntry[])
  if (entries.length === 0) return ''
  return entries
    .map((e) => `- [${e.id.slice(0, 8)}] (${e.category}) ${e.content.slice(0, 200)}`)
    .join('\n')
}

function normalizeRetroEntries(entries: RetroMemoryEntry[]): RetroMemoryEntry[] {
  const validCategories = new Set(['pattern', 'pitfall', 'preference', 'convention'])
  return entries
    .filter((e) => e.content?.trim())
    .map((e) => ({
      content: e.content.trim(),
      category: validCategories.has(e.category) ? e.category : 'pattern',
      tags: Array.isArray(e.tags) ? e.tags.map((t) => String(t).trim().toLowerCase()).filter(Boolean) : [],
      confidence: Math.max(0, Math.min(1, Number(e.confidence ?? 0.7))),
      supersedes: e.supersedes?.trim() || undefined,
      filePaths: Array.isArray(e.filePaths) ? e.filePaths.map((p) => p.trim()).filter(Boolean) : undefined,
    }))
}
