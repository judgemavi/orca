...
export interface BudgetedRetrievalResult {
  ...
  siblingTasks: Array<{ id: string; title: string; status: string; plan: string }>
  totalEntries: number
...
export async function retrieveBudgetedMemory(
  ...
  const prepareEntries = async (entries: MemoryEntry[]): Promise<MemoryEntry[]> => {
  ...
  const prepareEntry = async (entry: MemoryEntry): Promise<MemoryEntry | null> => {
  ...
  const exactMatches = (await prepareEntries(dedupeEntries(await memory.findByFilePaths(files))
    .filter((entry) => !seen.has(entry.id))))
    .slice(0, budgets.exact)
  for (const entry of exactMatches) seen.add(entry.id)

  const semanticMatches = (await prepareEntries((await memory
    .searchExcluding(query, budgets.fts * 2, []))
    .filter((entry) => !seen.has(entry.id))
    .slice(0, budgets.fts)))
  ...
  for (const tag of tags) {
    ...
    const rows = (await memory.list({ tag })).filter((entry) => !seen.has(entry.id))
  ...
  const recencyMatches = (await prepareEntries([...(await memory.list())]
    .reverse()
    .filter((entry) => !seen.has(entry.id))
    .slice(0, budgets.recency)))
  for (const entry of recencyMatches) seen.add(entry.id)

  const siblingCandidatesTasks = (await tasks
    .list())
    .filter((task) =>
...
export function buildMemoryContext(result: BudgetedRetrievalResult): string {
  ...
  if (result.summary && result.summary.content.trim()) {
    blocks.push(['## Project Context', '', result.summary.content.trim()].join('\n'))
  }
