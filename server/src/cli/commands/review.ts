...
export function registerReviewCommands(
  ...
  review
    ...
    .action(async (id: string | undefined, opts: { tool?: string; model?: string; prompt?: string }) => {
      const taskID = await resolveReviewTaskID(deps.taskStore, id)
...
async function resolveReviewTaskID(taskStore: TaskStore, id?: string): Promise<string> {
