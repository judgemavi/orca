...
export function registerExploreCommands(
  ...
  const cmd = program.command('explore').description('Explore context operations')
  ...
  cmd
    ...
    .action(async (opts: { tool?: string; model?: string }) => {
      ...
      const path = await writeExploreContext(repoDir, buildExploreContext(files))
      printJSON({ status: 'completed', path, files: files.length, tool, model })
    })
