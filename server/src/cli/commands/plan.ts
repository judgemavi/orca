...
import { availableTools, toolModels, type DriverRegistry } from '../../driver/registry'
...
export function registerPlanCommands(
  ...
  plan
    ...
    .action(async (opts: { goal?: string; tool?: string; model?: string }) => {
      ...
      if (!goal) {
        if (!canPrompt()) throw new Error('--goal is required in non-interactive mode')
      ...
      const model = (opts.model ?? '').trim() || await resolveModel(tool, models)

      const breakdown = await breakdownTask({ goal }, { taskStore: deps.taskStore })
      ...
      printJSON({ status: 'breaking_down', operationId: interaction.id, tool, model, proposed })
    })
  ...
  plan
    ...
    .action(async (opts: { operation?: string }) => {
      const operationID = await resolveOperationID(opts.operation, deps.interactionStore)
  ...
  plan
    ...
    .action(async (opts: { operation?: string }) => {
      const operationID = await resolveOperationID(opts.operation, deps.interactionStore)
      await rejectBreakdown('', operationID, { interactions: deps.interactionStore })
