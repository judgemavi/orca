...
import { refreshMemoryEntries, syncMemoryWithGit } from '../../domain/memory-sync'
import type { MemoryStore } from '../../store/memory'
import { printJSON } from '../format'
import { confirm, pickFromList } from '../helpers'

export function registerMemoryCommands(program: Command, repoDir: string, memory: MemoryStore) {
  ...
  cmd
    ...
    .action(async (opts: { category?: string; tag?: string; q?: string; limit: string }) => {
      const limit = Math.max(1, Math.min(Number.parseInt(opts.limit, 10) || 50, 500))
      if ((opts.q ?? '').trim()) {
  ...
  cmd.command('delete [id]')
    ...
    .action(async (id: string | undefined, opts: { yes?: boolean }) => {
      let entryID = (id ?? '').trim()
      if (!entryID) {
        if (!canPrompt()) throw new Error('id required in non-interactive mode')
...
async function browseMemory(memory: MemoryStore, rows: Awaited<ReturnType<MemoryStore['list']>>) {
  if (rows.length === 0) {
    printJSON([])
    return
  }
