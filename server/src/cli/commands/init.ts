...
import { pickFromList, confirm, ensureNotCancelled, textInput } from '../helpers'
...
export async function runInitCommand(
  ...
  const missing = registry.registered().filter((name) => !available.includes(name))
  ...
  if (autoYes || available.length === 1) {
    ...
    enabledTools = ensureNotCancelled(
      await multiselect({
        ...
        options: [
          ...available.map((name) => ({ value: name, label: `✓ ${name}`, hint: 'detected' })),
          ...missing.map((name) => ({ value: name, label: `✗ ${name}`, hint: 'not found', disabled: true })),
        ],
  ...
  const current = alreadyInitialized ? await loadExistingConfig(repoDir) : defaultConfig()
  ...
  const defaultTool = enabledTools.length === 1
    ? enabledTools[0]
    : autoYes
  ...
  const costBudget = costBudgetRaw
    ? Number.parseFloat(costBudgetRaw)
    : 0
  ...
  const supervisorTool = enabledTools.length === 1
    ? enabledTools[0]
    : autoYes
  ...
  for (const phase of ORCHESTRATOR_PHASES) {
    const phaseCurrentTool = current.orchestrator.phases[phase]?.tool || defaultTool
    const phaseTool = enabledTools.length === 1
      ? enabledTools[0]
      : autoYes
        ? phaseCurrentTool
        : await pickFromList(
    ...
    const phaseModels = phaseModelsRaw.length > 0 ? phaseModelsRaw : defaultModels
...
async function ensureGitPreflight(repoDir: string, autoYes: boolean): Promise<void> {
  const inRepo = await isGitRepo(repoDir)
  if (!inRepo) {
    if (!autoYes) {
      const ok = await confirm('No git repository detected. Run git init?', true)
      if (!ok) {
...
function buildDefaults(repoDir: string, current: Config) {
  const projectName = current.project.name || baseName(repoDir) || 'orca-project'
  ...
  if (current.quality.llmAlignment !== false) qualitySelected.push('llmAlignment')
  ...
  return {
    ...
    qualitySelected,
    supervisorTool: current.orchestrator.supervisorTool || current.defaultTool || 'claude',
    supervisorModel: current.orchestrator.supervisorModel || current.defaultModel || '',
  }
}
...
async function ensureGitignoreEntry(repoDir: string, entry: string): Promise<boolean> {
  const path = `${repoDir}/.gitignore`
  const current = (await Bun.file(path).text().catch(() => '')).trim()
...
async function isGitRepo(repoDir: string): Promise<boolean> {
  const result = await runGit(repoDir, ['rev-parse', '--is-inside-work-tree'], true)
