...
export async function runCLI(deps: {
  ...
  program.command('init').description('Initialize orca workspace (handled at startup)').action(() => {})
  ...
  program
    ...
    .action(
      async (
        ...
        if (canPrompt()) {
          const proceed = await confirm('Start all ready pending/planned tasks?', true)
          if (!proceed) {
  ...
  program
    ...
    .action(async (opts: { interaction?: string; task?: string; phase?: string; json?: boolean }) => {
      ...
      if (taskID) {
        ...
        if (interactions.length === 0) {
          throw new Error(`no interactions for task ${taskID}${phase ? ` phase=${phase}` : ''}`)
        }
        if (opts.json) {
          console.log(JSON.stringify(interactions, null, 2))
          return
        }
        const content = await deps.interactionStore.readLog(interactions[0].id)
        console.log(content)
        return
      }

      if (phase) {
        const interactions = await deps.interactionStore.listProjectByPhase(phase)
        if (opts.json) {
          console.log(JSON.stringify(interactions, null, 2))
          return
        }
      ...
      throw new Error('provide --interaction <id>, --task <id>, or --phase <phase>')
    })
  ...
  program
    ...
    .action(async (opts: { task?: string }) => {
      ...
      if (taskID) {
        if (canPrompt()) {
          const proceed = await confirm(`Remove logs for task ${taskID}?`, false)
          if (!proceed) {
  ...
  const context = program.command('context').description('Explore context file operations')
