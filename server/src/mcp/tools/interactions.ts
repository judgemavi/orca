  ...
  return [
    defineTool({
      ...
      handler: async (input) => {
        ...
        let items = phase ? await interactions.listByPhase(taskID, phase) : await interactions.list(taskID)
        if (status) {
    ...
    defineTool({
      ...
      handler: async (input) => {
        ...
        const phase = input.phase ?? ''
        return {
          interactions: phase ? await interactions.listByPhase(taskID, phase) : await interactions.list(taskID),
        }
      },
    }),
  ]
