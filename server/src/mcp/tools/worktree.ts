  ...
  return [
    ...
    defineTool({
      ...
      handler: async (input) => {
        ...
        if (dryRun) {
          return {
            removed: stale.map((item) => item.taskId),
            dryRun: true,
            errors: [],
          };
        }
        ...
        for (const item of stale) {
          const removeFromGit = await runGit(
            repoDir,
          ...
          if (removeFromGit.code !== 0) {
            errors.push(
              removeFromGit.stderr ||
                removeFromGit.stdout ||
                `failed removing ${item.path}`,
            );
            continue;
          }
