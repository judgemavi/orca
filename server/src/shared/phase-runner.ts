...
import { collectAssistantText, formatTemplate, resolvePhaseExecution } from '../domain/llm'
...
export async function createPhaseRunner(deps: PhaseDeps) {
  return async function runPhase<T>(
    ...
    if (!execution) {
      throw new Error(opts.resolveErrorMessage ?? `unable to resolve ${phase} tool/model`)
    }
    ...
    const finishInteraction = async (fields: InteractionFinishFields): Promise<void> => {
    ...
    try {
      ...
      if (runResult.exitCode !== 0) {
        ...
        throw new Error(runResult.error ?? `${label} exited with code ${runResult.exitCode}`)
      }
...
async function loadPhasePrompt(repoDir: string, opts: PhaseOpts): Promise<string> {
...
async function loadPromptWithName(repoDir: string, promptName?: PromptName): Promise<string> {
  if (!promptName) {
