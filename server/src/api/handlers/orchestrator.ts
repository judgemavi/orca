...
export function registerOrchestratorHandlers(
  ...
  app.get('/orchestrator/status', async (c) => {
    const session = await getActiveSession(db);
    if (!session) return c.json({ data: { active: false, sessionId: null } });
    return c.json({ data: { active: true, sessionId: session.id } });
  });
  ...
  app.get('/orchestrator/sessions', async (c) => {
    const sessions = await db
      .select({
        ...
        messageCount: count(schema.orchestratorMessages.id),
        preview: sql<string>`(
          SELECT content FROM orchestrator_messages
          WHERE session_id = ${schema.orchestratorSessions.id}
            AND role = 'user'
      ...
      .leftJoin(
        schema.orchestratorMessages,
        eq(
          schema.orchestratorMessages.sessionId,
          schema.orchestratorSessions.id,
        ),
      )
      .groupBy(schema.orchestratorSessions.id)
  ...
  app.post('/orchestrator/sessions/:id/resume', async (c) => {
    const targetId = c.req.param('id');

    const rows = await db
      .select()
      .from(schema.orchestratorSessions)
      .where(eq(schema.orchestratorSessions.id, targetId))
      .limit(1);
    const target = rows[0];
    ...
    sink.broadcast('orchestrator.session.resumed', { id: targetId });

    return c.json({ data: { id: targetId, status: 'active' } });
  });
        ...
        start(controller) {
          void (async () => {
            ...
            try {
              ...
              let assistantID =
                trailingAssistantID || state.lastAssistantMessageID;
              if (!assistantID && !state.sawStreamEvent) {
                assistantID = await insertAssistantMessage(
                  db,
                  session.id,
                  result.assistant.trim() ||
                    'No output received from supervisor process.',
                  result.sessionId ? { sessionId: result.sessionId } : {},
                );
              }
