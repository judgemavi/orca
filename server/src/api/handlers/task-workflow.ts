...
export function registerTaskWorkflowHandlers(
  ...
  app.post('/tasks/:id/approve', async (c) => {
    ...
    broadcast(deps.sink, 'task.updated', updated as unknown as Record<string, unknown>)
  ...
  app.post('/tasks/:id/request-changes', async (c) => {
    ...
    if (task.status !== 'review') {
      return c.json({ error: 'task must be in review status to request changes' }, 400)
    }
  ...
  app.post('/tasks/:id/ai-review', async (c) => {
    ...
    if (!task) return c.json({ error: 'task not found' }, 404)
    if (task.status !== 'review') {
      return c.json({ error: `task must be in review status, got ${JSON.stringify(task.status)}` }, 400)
    }

    const body = await parseBody<ToolModelBody & { prompt?: string }>(c.req)
    const runInteractions = await deps.interactionStore.listByPhase(taskID, 'run')
    const latest = runInteractions.find((item) =>
      item.status === 'completed' && Boolean(item.diff?.trim()))
    const diff = latest?.diff?.trim() ?? ''
    if (!diff) {
      return c.json({ error: 'no completed run interaction with diff found' }, 400)
    }
  ...
  app.post('/tasks/:id/breakdown/accept', async (c) => {
    ...
    try {
      if (proposed.length === 0) {
        const breakdown = await breakdownTask({ taskId: parentID }, { taskStore: deps.taskStore })
        proposed = breakdown.proposed
      }

      accepted = await acceptBreakdown(parentID, proposed, { taskStore: deps.taskStore, queue: deps.queue })
    } catch (error) {
      return errorResponse(c, error)
    }
    ...
    broadcast(deps.sink, 'task.updated', { id: parentID, status: 'broken_down' })
  ...
  app.post('/tasks/:id/breakdown/reject', async (c) => {
    ...
    return c.json({ data: { taskId: taskID, interactionId: interactionID, rejected: true } })
  })
}
