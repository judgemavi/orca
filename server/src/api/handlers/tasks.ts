...
import { deleteTask, DeleteWorkflowError } from '../../workflows/delete'
...
export function registerTaskHandlers(
  ...
  app.get('/tasks/ready', async (c) => c.json({ data: await taskStore.getReady() }))
  ...
  app.post('/tasks/:id/deps', async (c) => {
    const taskID = c.req.param('id')
    const body = await parseBody<{ dependsOn?: string; dependsOnIds?: string[] }>(c.req)
...
function validateManualStatusTransition(current: TaskStatus, next: TaskStatus): string | null {
