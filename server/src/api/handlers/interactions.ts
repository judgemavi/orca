...
export function registerInteractionHandlers(app: Hono, interactions: InteractionStore) {
  app.get('/tasks/:id/interactions', async (c) => {
    ...
    let data = phase ? await interactions.listByPhase(taskID, phase) : await interactions.list(taskID)
    if (status) {
